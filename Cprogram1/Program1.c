...
int main(){
	...
	FILE* sales_file = fopen("p1_input.txt", "r"); // file handling
...
// month indexes: 0-5, 1-6, 2-7, 3-8, 4-9, 5-10, 6-11
	...
	float sum2 = 0;
	for (int b = 1; b < 7; b++){ //find sum of feb to july
		sum2 = sum2 + sales[b];
	}
	float avg2 = sum2 / 6;
	//prints
	...
	float sum3 = 0;
	for (int c = 2; c < 8; c++){ //find sum of march to aug
		sum3 = sum3 + sales[c];
	}
	float avg3 = sum3 / 6;
	//prints
	...
	float sum4 = 0;
	for (int d = 3; d < 9; d++){ //find sum of april to sept
		sum4 = sum4 + sales[d];
	}
	float avg4 = sum4 / 6;
	//prints
	...
	float sum5 = 0;
	for (int e = 4; e < 10; e++){ //find sum of may to oct
		sum5 = sum5 + sales[e];
	}
	float avg5 = sum5 / 6;
	//prints
	...
	float sum6 = 0;
	for (int f = 5; f < 11; f++){ //find sum of may to oct
		sum6 = sum6 + sales[f];
	}
	float avg6 = sum6 / 6;
	//prints
	...
	float sum7 = 0;
	for (int g = 6; g < 12; g++){ //find sum of may to oct
		sum7 = sum7 + sales[g];
	}
	float avg7 = sum7 / 6;
	//prints
	printf("\n%-10s - %10s $%.2f\n", months[6], months[11], avg7);
//end scrolling report
